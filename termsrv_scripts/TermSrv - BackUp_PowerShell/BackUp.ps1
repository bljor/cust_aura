...
#$BackupRootPath = "\\aura.dk\services\Backup\ServerOS\Power" 
$BackupRootPath = "\\oee-sms.oestjysk-energi.dk\asr"
...
$WSBBackupFeature = Get-WindowsFeature | Where-Object {$_.Name -eq "Backup"} 
$WSBBackupFeatureTools = Get-WindowsFeature | Where-Object {$_.Name -eq "Backup-Tools"}
...
If ($WSBBackupFeature.Installed -and $WSBBackupFeatureTools.Installed -eq "True")
    ...
    echo ""
    $InstallWSB = Add-WindowsFeature -Name Backup-Features -IncludeAllSubFeature
...
function EmailNotification() 
    ...
    if (($BackupResult -eq 0) -and ($ErrorMessage -eq $Null)){
        ...
        {
        If ($ErrorMessage -ne $Null) {$errordesc = $ErrorMessage} else {$errordesc = (Get-WBJob -Previous 1).ErrorDescription}
    ...
    if ($SendEmail -eq 1 -and $BackupResult -ne 0 -or $ErrorMessage -ne $Null) {
    	echo ""
    	echo "Backup Failed, sending email"
    	echo ""
    	Send-MailMessage -to $to -from $from  -subject $Subject -body $Body -smtpserver $smtpserver -BodyAsHtml -attachments $attachments
    	}
    if ($SendEmail -eq 1 -and $BackupResult -eq 0 -and $ErrorMessage -eq $Null) {
        echo ""
...
echo ""
try {Start-WBBackup -Policy $WBPolicy}
catch {echo "Errors encountered"
