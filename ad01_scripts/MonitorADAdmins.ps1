<#===========================================================================================================================
 Script Name: MonitorADAdmins.ps1
...
$reportpath= "c:\scripts\report.html"

$to="c7fd8bc0.aura.dk@emea.teams.ms"
$from="it@aura.dk"
...
$events | ForEach-Object {
    ([xml]$_.ToXml()).Event.EventData | ForEach-Object {
       ...
       $props= New-Object -Type PSObject -Property @{
        ...
        Addedby=$props.subjectusername
        Addedto=$props.TargetUserName
...
$props |select User,Addedby,Addedto | ConvertTo-Html   -body "<H2>Find The Following Details</H2>"-Head $style| Out-File $reportpath
...
$MailMessage = @{ 
    To = $to
    From = $from
    Subject = "Attention a user is added to Administrative Group " 
    Body = "$body" 
    priority="high"
    Smtpserver = $smtp
