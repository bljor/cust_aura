...
var roleVirtualNetworksSubnetJoin = '5e434ec5-307c-5e24-8f25-7cf0296a9e69'    // ID of the custom role - cannot be reused, so find a way to obtain it dynamically or through a parameter from the deployment script

var roleVmUserLogin = 'fb879df8-f326-4884-b1cf-06f3ad86be52'            // Virtual Machine User Login role
var roleVmAdminLogin = '1c0163c0-47e6-4577-8991-ea5c82e286e4'           // Virtual Machine Admin Login role
...
// Grant access developers access to User Login to the Virtual Machine
resource roleAssignmentVMUser 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid('Role Assignment - let developers ${keyVaultDevelopersId} access ${vm.name} - (Role: Virtual Machine User Login)')
  scope: vm
  properties: {
...
resource roleAssignment001 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid('Role Assignment - let managed identity ${managedidentity001.name} access ${keyvaultcerts.name} - (Role: Key Vault Certificate user)')
  scope: keyvaultcerts
  properties: {
...
// Grant developers the custom role, that was deployed through the deploy-solution.ps1 script
// This functionality does not use the module that the other role assignements does... Should be
// cleaned up or re-written for consistency. The deployment scope for this role assignement is
// different than the deployment scope of the earlier role assignments

/*

resource roleAssignmentVirtualNetworks 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid(virtualNetwork.id, roleVirtualNetworksSubnetJoin, keyVaultDevelopersId)
  scope: resourceId('Microsoft.Network/virtualNetworks', virtualNetworkName)
  properties: {
    roleDefinitionId: roleVirtualNetworksSubnetJoin
    principalId: keyVaultDevelopersId
    principalType: 'Group'
  }
}
...
/* This was the original attempt to perform role assignements on a different scope
module roleAssignmentVirtualNetworks 'roleAssignment.bicep' = {
  name: guid(virtualNetwork.name, keyVaultDevelopersId,'[AURA] Network-VirtualNetworks-Subnets-Join')
  scope: virtualNetwork
  params: {
    roleAssignmentName: guid(resourceGroup().id, 'virtualNetworks', keyVaultDevelopersId, 'CustomRole')
    targetPrincipalId: keyVaultDevelopersId
    targetPrincipalType: 'Group'
    targetRoleId: roleVirtualNetworksSubnetJoin
  }
}
  */

// Grant the developers rbac admin access to resource group in parameter developersResourceGroup
